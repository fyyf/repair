...
$this->assign('count',$b);
	...
	public function set(){
		if(IS_POST){
			 $a=M('admin');
			   $admin_map1['admin_id']=$_SESSION['admin_id'];

  
    $b=$a->where($admin_map1)->find();
...
if(!empty($_POST['userphone'])){
	$user_data['phone']=$_POST['userphone'];
}
if(!empty($_POST['password'])){
	if(!empty($_POST['old_password'])){
   
    if($b['password']!=md5($_POST['old_password'])){
    $this->error('原密码输入错误');
    }else{
$admin_data1['password']=md5($_POST['password']);
 if(($admin_data1['password']!=$b['password'])){

$g=$a->where($admin_map1)->save($admin_data1);
}
...
$d=1;
$g=1;
if(($admin_data['email']!=$b['email'])){

$c=$a->where($admin_map1)->save($admin_data);
}
//dump(count($user_data));
$user_map['user_id']=$_SESSION['user_id'];
$f=$e->where($user_map)->find();
if(($user_data['name']!=$f['name']) OR ($user_data['phone']!=$f['phone'])){
$d=$e->where($user_map)->save($user_data);}
...
$this->error('修改失败');
			...
			$a=M('admin');
			$admin_map['admin_id']=$_SESSION['admin_id'];
			$b=$a->where($admin_map)->find();
			$c=M('userextend');
			$userextend_map['user_id']=$_SESSION['user_id'];
			$d=$c->where($userextend_map)->find();
			$this->assign('userextend',$d);
			$this->assign('admininfo',$b);
$this->display();}
	}
...
//$order_map['status']=0;
 	...
 	$b = $a->where($order_map)->page($p.','.$pagecount)->select();
